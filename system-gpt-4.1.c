...
void kernel_call(message *m_user, struct proc *caller) {
    ...
    if (m_user == NULL || caller == NULL) {
        printf("ERROR: NULL pointer passed to kernel_call\n");
        if (caller)
            cause_sig(proc_nr(caller), SIGSEGV);
        return;
    }
    ...
    if (copy_msg_from_user(m_user, &msg) != 0) {
        printf("WARNING: Invalid user pointer 0x%08x from process %s / %d\n",
...
void set_sendto_bit(const struct proc *rp, int id)
{
    ...
    struct priv *dest_priv;

    if (nr == NONE || priv_id(rp) == id) {
        unset_sys_bit(rp_priv->s_ipc_to, id);
        return;
    }

    set_sys_bit(rp_priv->s_ipc_to, id);

    dest_priv = priv_addr(id);
    if (dest_priv == NULL)
        return;

    if (dest_priv->s_trap_mask & ~((1 << RECEIVE)))
        set_sys_bit(dest_priv->s_ipc_to, priv_id(rp));
}
...
int send_sig(endpoint_t ep, int sig_nr)
{
    ...
    if (sigaddset(&prv->s_sig_pending, sig_nr) != 0)
        return EINVAL;
...
void cause_sig(proc_nr_t proc_nr, int sig_nr)
{
    ...
    is_pending = sigismember(&rp->p_pending, sig_nr);
    if (!is_pending) {
...
void send_diag_sig(void)
{
    ...
    for (privp = BEG_PRIV_ADDR; privp < END_PRIV_ADDR; privp++) {
        ...
        struct proc *proc_p = proc_addr(privp->s_proc_nr);
        if (proc_p == NULL) {
            continue;
        }
        ep = proc_p->p_endpoint;
        if (send_sig(ep, SIGKMESS) != 0) {
            // Handle error if needed (log, track, etc.)
            continue;
        }
    }
}
...
static void clear_ipc(struct proc *rc)
{
    ...
    if (RTS_ISSET(rc, RTS_SENDING)) {
        if (okendpt(rc->p_sendto_e, &target_proc) == OK) {
            struct proc *target = proc_addr(target_proc);
            if (target != NULL) {
                xpp = &target->p_caller_q;
                while (*xpp) {
                    if (*xpp == rc) {
                        *xpp = (*xpp)->p_q_link;
...
void kernel_call_resume(struct proc *caller)
{
    ...
    if (caller->p_vmrequest.saved.reqmsg.m_source != caller->p_endpoint) {
        return;
    }
...
#ifdef CONFIG_SMP
	if ((cpu != -1 && (cpu < 0 || (unsigned)cpu >= ncpus)) || (cpu != -1 && !cpu_is_ready(cpu)))
		return cpu != -1 && (cpu < 0 || (unsigned)cpu >= ncpus) ? EINVAL : EBADCPU;
#endif
...
{
    if (rp == NULL || (type != IPCF_BLACKLIST && type != IPCF_WHITELIST))
        return EINVAL;
    if (length == 0 || length % sizeof(ipc_filter_el_t) != 0)
        return EINVAL;

    int num_elements = (int)(length / sizeof(ipc_filter_el_t));
    if (num_elements <= 0 || num_elements > IPCF_MAX_ELEMENTS)
        return E2BIG;

    ipc_filter_t *ipcf = NULL;
...
void clear_ipc_filters(struct proc *rp)
{
    ...
    if (rp->p_endpoint == VM_PROC_NR && vmrequest != NULL) {
        if (send_sig(VM_PROC_NR, SIGKMEM) != OK) {
            panic("send_sig failed for VM_PROC_NR");
        }
    }
}
...
int check_ipc_filter(ipc_filter_t *ipcf, int fill_flags)
{
    ...
    if (ipcf->elements == NULL || ipcf->num_elements < 0)
        return EINVAL;
...
int allow_ipc_filtered_msg(struct proc *rp, endpoint_t src_e, vir_bytes m_src_v, message *m_src_p)
{
	...
	if (!m_src_p) {
		...
		get_mtype = 1;
#else
		ipc_filter_t *tmp = ipcf;
		while (tmp) {
			if (tmp->flags & IPCF_MATCH_M_TYPE) {
				get_mtype = 1;
				break;
			}
			tmp = tmp->next;
...
#endif
		if (get_mtype) {
			r = data_copy(src_e, m_src_v + offsetof(message, m_type), KERNEL,
...
#if DEBUG_DUMPIPCF
	printmsg(m_src_p, proc_addr(_ENDPOINT_P(src_e)), rp, allow ? '+' : '-', 1);
#endif
...
int priv_add_irq(struct proc *rp, int irq)
{
    ...
    priv_data->s_flags |= CHECK_IRQ;

    for (int i = 0; i < priv_data->s_nr_irq; i++) {
        if (priv_data->s_irq_tab[i] == irq)
            return OK;
    }

    if (priv_data->s_nr_irq >= NR_IRQ) {
        printf("do_privctl: %d already has %d irq's.\n", rp->p_endpoint, priv_data->s_nr_irq);
        return ENOMEM;
    }

    priv_data->s_irq_tab[priv_data->s_nr_irq] = irq;
    priv_data->s_nr_irq++;
    return OK;
}
...
int priv_add_io(struct proc *rp, struct io_range *ior)
{
    ...
    for (i = 0; i < priv_data->s_nr_io_range; i++) {
    ...
    if (priv_data->s_nr_io_range >= NR_IO_RANGE) {
        printf("do_privctl: %d already has %d i/o ranges.\n",
               rp ? rp->p_endpoint : -1, priv_data->s_nr_io_range);
        return ENOMEM;
    }
...
int priv_add_mem(struct proc *rp, struct minix_mem_range *memr)
{
    ...
    privp->s_flags |= CHECK_MEM;

    for (int i = 0; i < privp->s_nr_mem_range; i++) {
        if (privp->s_mem_tab[i].mr_base == memr->mr_base &&
            privp->s_mem_tab[i].mr_limit == memr->mr_limit) {
            return OK;
        }
    }

    if (privp->s_nr_mem_range >= NR_MEM_RANGE) {
        printf("do_privctl: %d already has %d mem ranges.\n",
               rp ? rp->p_endpoint : -1, privp->s_nr_mem_range);
        return ENOMEM;
    }

    privp->s_mem_tab[privp->s_nr_mem_range] = *memr;
    privp->s_nr_mem_range++;
    return OK;
}
